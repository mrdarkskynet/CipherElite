...
# Environment whitelist for subprocess (SECURITY: prevents bot secrets leaking to user scripts)
SAFE_ENV_KEYS = {
...
def build_subprocess_env(user_folder: str) -> dict:
    """SAFE env: whitelisted keys + user's own .env. Bot secrets never leak."""
    env = {}
    ...
    env.setdefault('HOME', user_folder)
...
def handle_config_file(file_path, script_owner_id, user_folder, file_name, message):
    """Just save config files. Do NOT run them."""
    try:
    ...
    try:
        ...
        try:
            ...
            if file_size == 0:
                bot.send_message(
                    chat_id_for_reply,
                    f"📜 Logs for `{file_name}` (User `{script_owner_id}`):\n\n_(empty — script produced no output yet)_",
                    parse_mode='Markdown',
                )
                return

            # ---------- LARGE LOG → send as .txt file ----------
            if file_size > 3500:
                ...
                bio.write(log_bytes)
                bio.seek(0)
                bio.name = f"{os.path.splitext(file_name)[0]}.log"
                try:
                    bot.send_document(
                        chat_id_for_reply,
                        bio,
            ...
            header = f"📜 Logs for `{file_name}` (User `{script_owner_id}`):\n\n"
            body = f"```\n{safe_log}\n```"
            ...
            if "message is too long" in err:
                bot.send_message(
                    chat_id_for_reply,
                    f"❌ Log for `{file_name}` is too large to display inline. "
                    f"Try again — it should be sent as a file.",
                    parse_mode='Markdown',
                )
            else:
