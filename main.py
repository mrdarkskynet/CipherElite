...
# --- Configuration ---
TOKEN = '8647741274:AAGqNgypGyDnY37IqAYfy2QgGqb9L48cHLo'
OWNER_ID = 5076047031
ADMIN_ID = 5076047031
YOUR_USERNAME = '@mrdarkvipx'
UPDATE_CHANNEL = 'https://t.me/numbervirtualfast'
...
# Environment whitelist for subprocess (SECURITY FIX: prevents bot secrets leaking to user scripts)
SAFE_ENV_KEYS = {
...
def sanitize_filename(name: str) -> str:
    """Sanitize a filename so it can NEVER escape its user folder."""
    if not name:
        return ""
...
def classify_file(filename: str):
    ...
    # Dotfiles: .env, .env.local, .gitignore, ...
    if lower.startswith('.'):
        if lower == '.env' or lower.startswith('.env.'):
            return ('config', lower)
        # Any other dotfile is treated as config (safe to store, not to run)
        return ('config', lower)
...
def load_user_env(user_folder: str) -> dict:
    """Load user's own .env file (their folder only)."""
    env = {}
    ...
    try:
        ...
        logger.error(f"Error loading .env from {user_folder}: {e}")
    return env


def build_subprocess_env(user_folder: str) -> dict:
    """Build a SAFE env for subprocess: only whitelisted keys + user's own .env.
    The bot's own secrets (BOT_TOKEN, LAMIX_API_TOKEN, etc.) NEVER leak to
    user scripts, so users cannot interfere with each other or with the host."""
    env = {}
    ...
    env.setdefault('HOME', user_folder)  # make $HOME point at user's folder
    env.setdefault('PWD', user_folder)
    # User's own .env overrides / adds
    env.update(load_user_env(user_folder))
    return env
...
def get_user_file_count(user_id):
    """Count ONLY script files against the limit. Config files are free."""
    files = user_files.get(user_id, [])
    return sum(1 for _, ftype in files if ftype in ('py', 'js'))
...
def run_script(script_path, script_owner_id, user_folder, file_name, message_obj_for_reply):
    ...
    try:
        ...
        try:
            ...
            # SECURITY: only pass safe env + user's own .env
            user_env = build_subprocess_env(user_folder)
...
def handle_zip_file(downloaded_file_content, file_name_zip, message):
    ...
    try:
        ...
        if 'requirements.txt' in extracted_items:
            ...
            bot.reply_to(message, "ℹ️ Found `requirements.txt` — dependencies are already installed. Skipping install.")

        if 'package.json' in extracted_items:
            logger.info("package.json found in zip — IGNORING (deps already installed)")
            bot.reply_to(message, "ℹ️ Found `package.json` — Node dependencies are not installed from uploaded zips.")
...
def handle_config_file(file_path, script_owner_id, user_folder, file_name, message):
    """Just save config files. Do NOT run them. Also strip leading 'config' from log names."""
    try:
        _, ext = classify_file(file_name)
        save_user_file(script_owner_id, file_name, ext.lstrip('.') or 'config')
        bot.reply_to(
            message,
            f"✅ Saved config file: `{file_name}`\n"
            f"📁 Stored in your folder. It will be available when your script runs.",
            parse_mode='Markdown'
        )
        logger.info(f"Saved config file {file_name} for user {script_owner_id}")
    except Exception as e:
...
def _logic_send_welcome(message):
    ...
    welcome_msg_text = (f"〽️ Welcome, {user_name}!\n\n🆔 Your User ID: `{user_id}`\n"
                        ...
                        f"   Config files (.env/.json/.txt) are free and don't count against your limit.\n\n"
...
def _logic_upload_file(message):
    ...
    if current_files >= file_limit:
        limit_str = str(file_limit) if file_limit != float('inf') else "Unlimited"
        bot.reply_to(message, f"⚠️ Script limit ({current_files}/{limit_str}) reached. Delete scripts first.")
        return
    bot.reply_to(
        message,
        "📤 Send your file:\n"
...
def _logic_check_files(message):
    ...
    for file_name, file_type in sorted(user_files_list):
        ...
        status_icon = "🟢 Running" if is_running else ("📄 Config" if file_type not in ('py', 'js') else "🔴 Stopped")
...
def _logic_run_all_scripts(message_or_call):
    ...
    for target_user_id, files_for_user in all_user_files_snapshot.items():
        ...
        for file_name, file_type in files_for_user:
            # Skip config files (don't "run" them)
            ...
            if not is_bot_running(target_user_id, file_name):
                ...
                if os.path.exists(file_path):
                    ...
                    logger.warning(f"File '{file_name}' for user {target_user_id} not found. Skipping.")
...
def handle_file_upload_doc(message):
    ...
    if category == 'unknown':
        bot.reply_to(
            ...
            f"✅ Allowed: `.py`, `.js`, `.zip`, `.env`, `.json`, `.yaml`, `.yml`, `.toml`, `.ini`, `.cfg`, `.txt`, `.xml`, `.sh`, `.csv`, `.md`, `.sql`, `.pem`, `.key`, `.crt`",
            parse_mode='Markdown'
        )
        return

    # Only script files count against the limit
    if category == 'script':
        file_limit = get_user_file_limit(user_id)
        current_files = get_user_file_count(user_id)
        if current_files >= file_limit:
            limit_str = str(file_limit) if file_limit != float('inf') else "Unlimited"
            bot.reply_to(message, f"⚠️ Script limit ({current_files}/{limit_str}) reached. Delete scripts via /checkfiles.")
            return
    ...
    try:
        ...
        if category == 'archive':
            ...
            logger.info(f"Saved script file to {file_path}")
            if ext == '.js':
            ...
            with open(file_path, 'wb') as f:
                f.write(downloaded_file_content)
            logger.info(f"Saved config file to {file_path}")
            handle_config_file(file_path, user_id, user_folder, file_name, message)
...
def upload_callback(call):
    ...
    if current_files >= file_limit:
        limit_str = str(file_limit) if file_limit != float('inf') else "Unlimited"
        bot.answer_callback_query(call.id, f"⚠️ Script limit ({current_files}/{limit_str}) reached.", show_alert=True)
        return
...
def file_control_callback(call):
    try:
        ...
        # Config files have no run/stop controls — just show info
        if file_type not in ('py', 'js'):
            bot.answer_callback_query(call.id)
            try:
                user_folder = get_user_folder(script_owner_id)
                cfg_path = os.path.join(user_folder, file_name)
                size = os.path.getsize(cfg_path) if os.path.exists(cfg_path) else 0
                bot.edit_message_text(
                    f"📄 Config file: `{file_name}`\n"
                    f"📦 Size: `{size}` bytes\n"
                    f"👤 Owner: `{script_owner_id}`\n"
                    f"ℹ️ This file is available to your scripts at runtime.",
...
def process_remove_subscription_id(message):
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Failed to notify user of sub removal: {e}")
    except ValueError:
        bot.reply_to(message, "⚠️ Invalid ID. Send numerical ID or /cancel.")
